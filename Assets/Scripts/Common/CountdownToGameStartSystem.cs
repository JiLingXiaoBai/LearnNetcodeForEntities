...
public partial class CountdownToGameStartSystem : SystemBase
{
    ...
    protected override void OnUpdate()
    {
        ...
        foreach (var (gameStartTick, entity) in SystemAPI.Query<RefRO<GameStartTick>>().WithAll<Simulate>()
                     .WithEntityAccess())
        {
            if (currentTick.Equals(gameStartTick.ValueRO.Value) || currentTick.IsNewerThan(gameStartTick.ValueRO.Value))
            {
            ...
            {
                var ticksToStart = gameStartTick.ValueRO.Value.TickIndexForValidTick -
